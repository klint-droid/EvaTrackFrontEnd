...
export default function QrScannerModal({
    ...
    return createPortal(
        <div className="fixed inset-0 w-screen h-screen flex justify-center items-center z-[9999] p-4">
            ...
            <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700 flex flex-col">
                {/* Modal Header */}
                <div className="w-full flex items-center justify-between bg-white dark:bg-slate-900 px-5 py-4 border-b border-slate-150">
                    <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        <QrCode size={18} className="text-slate-700 dark:text-slate-200" />
                        Scan Household QR Code
                    </h3>
                    <button
                        onClick={() => setQrModalOpen(false)}
                        className="p-1 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 rounded-full transition-all"
                    >
                        <X size={18} />
                    </button>
                </div>
                ...
                {/* Viewport Status Row */}
                <div className="w-full bg-slate-50 dark:bg-slate-800/50 border-b border-slate-150 py-3.5 px-5 text-center flex flex-col items-center justify-center gap-2">
                ...
                <div className="w-full bg-white dark:bg-slate-900 px-5 py-4 flex items-center justify-between gap-4">
                    <button
                        onClick={() => setQrModalOpen(false)}
                        className="inline-flex items-center gap-1.5 px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:bg-slate-800/50 rounded-lg text-xs font-semibold transition-all"
                    >
                        <X size={14} />
                        Cancel
                    </button>
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-950 text-white hover:bg-slate-900 active:scale-95 rounded-lg text-xs font-semibold transition-all shadow-sm dark:shadow-none"
                    >
                        <Keyboard size={14} />
                        Enter ID Manually
                    </button>
