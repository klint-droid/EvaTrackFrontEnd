  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">Active Operations</h2>
      </div>
      
      {activeEvents.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-xl p-10 text-center">
          <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center mx-auto mb-3">
            <CheckCircle2 className="w-6 h-6 text-emerald-500" />
          </div>
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">All Clear</h3>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {activeEvents.map(event => {
            ...
            return (
              <div key={event.event_id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-xl overflow-hidden hover:border-slate-300 dark:border-slate-600 transition-colors">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">{event.name}</h3>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {event.evacuation_centers.map(center => (
                          <span 
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-800 transition-colors"
                          >
