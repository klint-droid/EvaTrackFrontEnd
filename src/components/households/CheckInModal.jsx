                    ...
                    {isPersonnelUser ? (
                        <div>
                            <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">
                                ...
                                <br/>
                                <strong className="text-slate-800 dark:text-slate-100">{personnelEvacuation?.center?.name || 'Your Assigned Center'}</strong>
                            </p>
                            <div className="flex justify-end gap-3">
                                <button
                                    onClick={() => setCheckInModal({ open: false, member: null })}
                                    className="px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800/50 rounded-lg transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                        ...
                        <div>
                            <p className="text-sm text-slate-600 dark:text-slate-300 mb-3">Select the evacuation center to check them into:</p>
                            <div className="space-y-2 mb-6">
                                {allActiveEvacuations.map(evac => (
                                    <button
                                        ...
                                        disabled={statusUpdatingMemberId === checkInModal.member.member_id}
                                        className="w-full text-left px-4 py-3 border border-slate-200 dark:border-slate-700 hover:border-blue-300 hover:bg-blue-50 rounded-xl transition-all flex items-center justify-between group"
                                    >
                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200 group-hover:text-blue-700">
